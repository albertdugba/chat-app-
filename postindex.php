...
error_reporting(1);
//echo phpversion();
$connect = new mysqli("localhost", "root", "", "alphaDB");
if ($_POST['insertdata'] == "insertdata") {
    //print_r($_post);
    $insertString = "INSERT INTO `tbl`(`messagebody`,`fk_senderid`,`tech_receiver_ID`)
    ...
    $queryString = "SELECT * FROM `tbl` where  `fk_senderid` = '" . $_SESSION['userID'] . "' AND `tech_receiver_ID` ='" . $_SESSION['receiverID'] . "' OR `fk_senderid` = '" . $_SESSION['receiverID'] . "' AND `tech_receiver_ID` ='" . $_SESSION['userID'] . "' ORDER BY id ASC";
...
//$array = array("state"=>"success","message"=>$output);
    ...
    if (empty($_SESSION['receiverID'])) {$_SESSION['receiverID'] = 3;}
    $queryString = "SELECT * FROM `tbl` where  `fk_senderid` = '" . $_SESSION['userID'] . "' AND `tech_receiver_ID` ='" . $_SESSION['receiverID'] . "' and status = 0 OR `fk_senderid` = '" . $_SESSION['receiverID'] . "' AND `tech_receiver_ID` ='" . $_SESSION['userID'] . "' and status = 0 ORDER BY id ASC";
    $runquery = $connect->query($queryString);
    ...
    if (empty($_SESSION['receiverID'])) {$_SESSION['receiverID'] = 3;}
    $queryString = "UPDATE `tbl` SET status = 1 where  `fk_senderid` = '" . $_SESSION['userID'] . "' AND `tech_receiver_ID` ='" . $_SESSION['receiverID'] . "' AND `id` NOT IN (" . $_SESSION['messageID'] . ")
OR `fk_senderid` = '" . $_SESSION['receiverID'] . "' AND `tech_receiver_ID` ='" . $_SESSION['userID'] . "' AND `id` NOT IN (" . $_SESSION['messageID'] . ") ORDER BY id ASC";
    $runquery = $connect->query($queryString);
